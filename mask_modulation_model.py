...
def mask_init(masknum):
    ...
    if tc.MASK_PHASE_MODULATION is True:
        if tc.MASK_INIT_TYPE == 'const':
            ...
            mask_phase_org = tf.compat.v1.get_variable('mask_phase' + str(masknum), initializer=tf.random_uniform(shape=[tc.MASK_ROW, tc.MASK_COL],minval=0.5,maxval=1.5))
            mask_phase = mask_phase_org * 2 * np.pi
...
def inference(field):
    ...
    with tf.name_scope('Hybrid'):
        ...
        sensor_mean, sensor_variance = tf.nn.moments(roi,axes=[1,2],keepdims=True)
        ...
        measurement_cnn = tf.nn.avg_pool(input=measurement_cnn, ksize=[1,10,10,1], strides=[1,10,10,1], padding='VALID')
...
#    #BUCKET LOSS
#    self = tf.divide(self,tf.compat.v1.reduce_sum(self,-1,keep_dims=True))
#    self_64 = tf.cast(self, dtype=tf.float64)
#    loss_ = tf.reduce_mean(tf.square(1-tf.compat.v1.reduce_sum(tf.multiply(self_64,ground_truth),-1)))
    
#    NORMALIZED MSE LOSS
#     corr = tf.compat.v1.reduce_sum(tf.multiply(self,ground_truth))
#     autoCorrIn = tf.compat.v1.reduce_sum(tf.multiply(self,self))
# #    autoCorrGT = tf.compat.v1.reduce_sum(tf.multiply(ground_truth,ground_truth))
#     scaling_factor = tf.divide(corr,autoCorrIn)
#     self = tf.multiply(self,scaling_factor)
#     squared_deltas = tf.square(self - ground_truth)
#     loss = tf.compat.v1.reduce_sum(squared_deltas, reduction_indices=[1,2])
#     loss_ = tf.reduce_mean(loss, name='mse')

    #UNITARY MSE LOSS
#    energyratio = tf.divide(tf.compat.v1.reduce_sum(tf.square(tf.abs(ground_truth))),tf.compat.v1.reduce_sum(self))
#    ground_truth = tf.multiply(self,energyratio)
#    squared_deltas = tf.square(self - ground_truth)
#    loss = tf.compat.v1.reduce_sum(squared_deltas, reduction_indices=[1])
#    loss_ = tf.reduce_mean(loss, name='mse')
    
    #LEAKAGE LOSS
#    squared_delta_leak = tf.compat.v1.reduce_sum(tf.square(leakage),reduction_indices=[2])
...
# Cross entropy
    loss = tf.compat.v1.losses.sparse_softmax_cross_entropy(labels=ground_truth, logits=self)
...
def tv_loss_function(measurement):
    ...
    sum_axis = [1, 2]
    tot_var = (tf.compat.v1.reduce_sum(tf.abs(pixel_dif1), axis=sum_axis) +
               tf.compat.v1.reduce_sum(tf.abs(pixel_dif2), axis=sum_axis))
    tot_var_mean = tf.compat.v1.reduce_mean(tot_var, name='tv')
...
def training(loss):
    ...
    ONN_variable = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, 'mask')
    DNN_variable = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, 'conv') + tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, 'dense')
    extra_update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)

    if tc.OPTIMIZER == 'gradient':
        optimizer = tf.compat.v1.train.GradientDescentOptimizer(tc.LEARNING_RATE)
    elif tc.OPTIMIZER == 'adam':
        optimizer_ONN = tf.compat.v1.train.AdamOptimizer(tc.LEARNING_RATE * 10)
        optimizer_DNN = tf.compat.v1.train.AdamOptimizer(tc.LEARNING_RATE * 0.1)
    else:
        pass
...
def reset_ONN():
    ...
    train_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, 'mask')
